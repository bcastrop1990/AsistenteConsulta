...
import com.senasa.bpm.ng.masajes.dao.MasajesUsuarioDao;
...
@Service
@Slf4j
public class MasajesUsuarioDaoImpl implements MasajesUsuarioDao {
    @Autowired
    ...
    public String crearUsuario(Usuario request) {
        ...
        if (secondaryJdbcTemplate.queryForObject(sqlCount, Integer.class, email) >= 1){
        ...
        return String.valueOf(secondaryJdbcTemplate.update(sql, email, request.getPassword(), request.getNombres(), request.getApellidos(), request.getRol(), request.getEmpresa_id()));
    }
